...
def single_gpu_test(model,
    ...
    def count_parameters(model):
        ...
        for name, parameter in model.named_parameters():
            ...
            total_params+=params
        print(table)
        data = table.get_string(fields=["Modules", "Parameters"])
        text_byte = data.encode('utf-8')
        text_byte2 = str(total_params).encode('utf-8')

        return total_params
    
    print("Total Params: {}".format(count_parameters(model)))
    ...
    for i, data in enumerate(data_loader):
        with torch.no_grad():
            result = model(return_loss=False, pseudo_save_path=pseudo_save_path, **data)
