...
class EncoderDecoder(BaseSegmentor):
    ...
    def simple_test(self, img, img_meta, pseudo_save_path, rescale=True):
        ...
        if hasattr(self.decode_head, 'debug_output_attention') and \
            ...
            seg_prob, _ = seg_logit.max(dim=1) 

        pseudo_pred = seg_pred.clone() 
        pseudo_prob = seg_prob.clone() 
        ...
        if pseudo_save_path != '':
            for i in range(4): 
                pseudo_pred[(pseudo_prob < 0.9) * (pseudo_pred == i)] = 255.

            pseudo_pred = pseudo_pred.cpu().numpy() 

            plabel = np.asarray(pseudo_pred.squeeze(0), dtype=np.uint8) 
            plabel_col = colorize_mask(plabel) 
            os.makedirs(pseudo_save_path, exist_ok=True)
            plabel_col.save(os.path.join(pseudo_save_path, img_meta[0]['ori_filename'])) 
