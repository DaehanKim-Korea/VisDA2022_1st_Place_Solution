...
class GaussNoise(object):
    ...
    def __init__(self, prob):
        ...
        self.transform = A.Compose([
            A.GaussNoise(p=self.prob),
        ])

    def __call__(self, results):

        src = results['img'].copy()
        
        transformed = self.transform(image=src)
...
class RandomGridShuffle(object):
    ...
    def __init__(self, prob, grid):
        ...
        self.grid = grid
        self.transform = A.Compose([
            A.RandomGridShuffle(grid=self.grid, p=self.prob),
        ])

    def __call__(self, results):

        src = results['img'].copy()
        src_mask = results['gt_semantic_seg'].copy()

        transformed = self.transform(image=src, mask=src_mask)
